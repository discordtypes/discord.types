...
import { RateLimitError, RestError } from './Exceptions/';
	...
	public makeRateLimit(rateLimitData: RateLimit) {
		this.manager.debug(
			...
			[
				...
				`This rate limit is reset after ${
					rateLimitData.reset_after - Date.now()
				}`,
			].join('\n'),
		);
		...
		if (this.manager.options.thrownRateLimit)
			throw new RateLimitError(
				"A rate limit was expected, it's datas: " +
					[
						`The fullroute: ${rateLimitData.fullroute}`,
						`Global ? : ${rateLimitData.global}`,
						`The route hash: ${rateLimitData.hash}`,
	...
	public async runRequest(
		...
		while (this.limited) {
			...
			isGlobal
				? this.manager.debug(
						DEBUG,
						`A global rate limit expected. Blocking all requests for ${timeout}ms`,
				  )
				: this.manager.debug(
						DEBUG,
						`A local rate limit expected, retrying in ${timeout} ms`,
				  );
			await sleepTimeout;
		}
		...
		if (
			...
			this.manager.globalRemaining =
				this.manager.options.globalRequetsPerSecond;
		}

		this.manager.options.listenRequests ??
			this.manager.debug(
				'requests',
				`A request will be sent. It's datas: ` +
					[
						`Method: ${method}`,
						`FullUrl: ${url}`,
		...
		try {
			res = await fetch(url, {
				agent: this.manager.options.httpAgent,
		...
		this.manager.options.debugRequest ??
			this.manager.debug(
				REQUEST,
				`A request response has just been received. Datas: ${JSON.stringify(
					res.clone(),
				)}`,
			);
		let retryAfter = 0;
		...
		if (hash && this.hashId !== hash) {
			this.manager.hashes.set(`${method}:${hash}`, {
				hash: hash,
				lastAccess: Date.now(),
			});
			this.manager.debug(
				DEBUG,
				`Receive a new hash. Old hash: ${this.hashId} New hash: ${hash}`,
			);
		} else if (hash) {
		...
		if (
			this.manager.options.invalidRequestsWarningInterval > 0 &&
			invalidRequestCount %
				this.manager.options.invalidRequestsWarningInterval ===
				0
		) {
			this.manager.debug(
				DEBUG,
				`Too much invalid requests are submitted. Datas: `,
				[
		...
		if (res.ok) {
			...
			throw new RestError(
				`${res.statusText}, ${res.constructor.name}, ${method}, ${url}`,
			);
		} else {
			...
			throw new RestError(
				`DiscordAPIError: An unexpected error just happened, response data: ${JSON.stringify(
					await res.json(),
				)}`,
				0,
			);
